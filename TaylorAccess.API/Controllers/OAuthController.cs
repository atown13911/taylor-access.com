...
public class OAuthController : ControllerBase
{
    ...
    public async Task<ActionResult> CreateAppRole(string clientId, [FromBody] CreateAppRoleRequest request)
    {
        ...
        if (exists) return BadRequest(new { error = "A role with this name already exists for this application" });

        var role = new AppRole { AppClientId = clientId, Name = request.Name, Description = request.Description, Permissions = request.Permissions ?? "[]" };
    ...
    {
        var assignment = await _context.AppRoleAssignments.FirstOrDefaultAsync(a => a.UserId == userId && a.AppClientId == appClientId);
        if (assignment == null) return NotFound(new { error = "User does not have access to this application" });
        assignment.IsSuperAdmin = request.IsSuperAdmin;
        assignment.UpdatedAt = DateTime.UtcNow;
        await _context.SaveChangesAsync();
        await _auditService.LogAsync("app_superadmin_toggled", "AppRoleAssignment", assignment.Id, $"User {userId} super admin for {appClientId} set to {request.IsSuperAdmin}");
    ...
    private async Task<ActionResult<TokenResponse>> HandleAuthorizationCodeGrant(string? code, string? redirectUri, string? clientId, string? clientSecret)
    {
        ...
        // Generate tokens with app-specific role
        var accessToken = _jwtService.GenerateTokenForApp(user, clientId!);
    ...
    private async Task<ActionResult<TokenResponse>> HandleRefreshTokenGrant(string? refreshToken, string? clientId, string? clientSecret)
    {
        ...
        var newAccessToken = _jwtService.GenerateTokenForApp(user, token.ClientId);
