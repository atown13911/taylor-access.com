...
@Component({
        ...
        <div class="modal-overlay" (click)="showCreate.set(false)">
          <div class="modal" (click)="$event.stopPropagation()">
            ...
            <div class="form-row">
              ...
              <input type="text" [(ngModel)]="draft.position" />
            </div>
            <div class="form-row">
              <label>Source</label>
  ...
  styles: [`
    ...
    .form-row { display: flex; flex-direction: column; gap: 6px; margin-bottom: 10px; label { color: #8aa0b8; font-size: 0.8rem; } input, textarea { background: #111827; color: #d1d5db; border: 1px solid #2a2a4e; border-radius: 8px; padding: 8px 10px; } }
...
export class ApplicantsComponent implements OnInit {
  ...
  saveDraft(): void {
    ...
    const next: ApplicantRow = {
      id: Date.now(),
      fullName,
      position: String(this.draft.position || '').trim(),
