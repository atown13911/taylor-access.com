"""Probe: check gmail.readonly delegation and that the Gmail API is enabled.

Run: railway run python scripts/probe_gmail_scope.py
Read-only (token exchange + one messages.list). Never prints key material.
"""
import base64
import json
import os
...
def main():
    ...
    url = f"https://gmail.googleapis.com/gmail/v1/users/{urllib.parse.quote(SUBJECT)}/messages?maxResults=1"
    req = urllib.request.Request(url, headers={"Authorization": f"Bearer {token}"})
    try:
        with urllib.request.urlopen(req) as res:
            body = json.load(res)
        print(f"messages.list OK — estimate {body.get('resultSizeEstimate')} messages for {SUBJECT}")
    except urllib.error.HTTPError as e:
        print("messages.list FAILED", e.code)
